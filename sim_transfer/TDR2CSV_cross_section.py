...
def MakeCrossSection(
    ...
    del_limit_list = [list(df_project.index.values),
                      list(df_project['DRIFTT'].values),
                      list(df_project['TotThick'].values)
                      ]
    # print(del_limit_list)

    # 空のdict作成。
    # このheaderというdictにdatasetsで指定した情報をHDFから読み取り追加
    header = {'Project_ID': [], 'X': [], 'Y': [],
              'ElectrostaticPotential_Diff_Vf-0V': []}
    for i in range(len(datasets)):
        for j in range(len(datasets[i])):
            header[datasets[i][j]] = []

    # path内にある指定ファイルを全て取得しhdfpathsに格納
    ...
    # hdfpathsに格納されたファイルパスのうち、node_listにあるもののみhdfpath_dictに
    # ノード番号をキーとして格納
    hdfpath_dict = {}
    ...
    # node_listでforループまわしてhdfpath_dictにある場合は内部データをheaderに格納していく。
    # node_list[0][i]（構造ファイル）がない場合、スキップして次のnode_list[0][i]へ。
    # node_list[0][i]（構造ファイル）はあるがIfVf、BVファイルがない場合、NANデータを形成してスタック。

    for i in tqdm(range(len(node_list[0]))):
        if node_list[0][i] in hdfpath_dict.keys():
            # print('\noperating', node_list[0][i])
            with h5py.File(hdfpath_dict[node_list[0][i]], 'r') as f:
                ...
                header['Project_ID'] += [Project_ID_list[i]] * row_limit

            if node_list[1][i] in hdfpath_dict.keys():
                # print('\noperating', node_list[1][i])
                        ...
                        if colname[:-5] in dataset_dict.keys():
                            ...
                            if len(data) == row_limit:
                                try:
                                    ...
                                    header[colname] += [np.nan] * row_limit
                                if colname == 'ElectrostaticPotential_@_Vf':
                                    try:
                                        with h5py.File(hdfpath_dict[
                                                           node_list[1][
                                                               i]].replace(
                                                '_0012_des.tdr',
                                                '_0000_des.tdr'), 'r') as f0:
                                            data0 = list(f0[dataset_dict[
                                                colname[:-5]][0]][()])
                                            header[
                                                'ElectrostaticPotential_Diff_Vf-0V'] += [
                                                d - d0 for (d, d0) in
                ...
                for colname in datasets[1]:
                    header[colname] += [np.nan] * row_limit
                header['ElectrostaticPotential_Diff_Vf-0V'] += [
                                                                   np.nan] * row_limit

            if node_list[2][i] in hdfpath_dict.keys():
                # print('\noperating', node_list[2][i])
        ...
        # header各列の行数が同じにならなかった場合、エラー発生のノード番号を返して終了
        lens = map(len, header.values())
        if len(set(lens)) != 1:
            print('lengths are different at node=', str(node_list[0][i]))
            quit()
    ...
    for i in range(len(del_limit_list[0])):
        drop_index = df.index[(df['Project_ID'] == del_limit_list[0][i])
                              & (df['X'] > del_limit_list[1][i] + 1)
                              & (df['X'] < del_limit_list[2][i] - 1)]
        df = df.drop(drop_index)

    col = df.columns.tolist()  # 列名のリスト
    col.remove('ElectrostaticPotential_Diff_Vf-0V')  # 指定列を削除
    col.insert(col.index('ElectrostaticPotential_@_Vf'),
               'ElectrostaticPotential_Diff_Vf-0V')  # 指定列1の後に指定列2を追加
    df = df[col]
    df = df.sort_values(['Project_ID', 'X', 'Y'])
...
if __name__ == '__main__':
    ...
    df_cross_section = MakeCrossSection(
        ...
        datasets=datasets_l, save=True, output=r'df_cross_section.csv')
